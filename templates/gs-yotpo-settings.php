...
function gs_display_yotpo_admin_page() {
	...
	if(gs_yotpo_compatible()) {			
		...
		elseif (isset($_POST['yotpo_settings'])) {
			...
			gs_proccess_yotpo_settings();
			gs_display_yotpo_settings();
		}
		elseif (isset($_POST['yotpo_register'])) {
			check_admin_referer( 'yotpo_registration_form' );
			$success = gs_proccess_yotpo_register();
			if($success) {			
				gs_display_yotpo_settings();
			}
			else {
				gs_display_yotpo_register();	
			}
...
function gs_display_yotpo_settings() {
	$yotpo_settings = get_option('yotpo_settings', gs_yotpo_get_default_settings());
	...
	$google_tracking_params = '&utm_source=yotpo_plugin_woocommerce&utm_medium=header_link&utm_campaign=getshopped_customize_link';
	if (!empty($yotpo_settings['app_key']) && !empty($yotpo_settings['secret'])) {
...
function gs_proccess_yotpo_settings() {
	$current_settings = get_option('yotpo_settings', gs_yotpo_get_default_settings());
...
function gs_proccess_yotpo_register() {
	$errors = array();
        ...
        try {        	        	
        	...
        	if(!empty($response['status']) && !empty($response['status']['code'])) {
        		if($response['status']['code'] == 200) {
        			...
        			$account_platform_response = $yotpo_api->create_account_platform(array( 'shop_domain' => gs_yotpo_get_shop_domain(),
        			...
        			if(!empty($response['status']['message'])) { 
        				if(!empty($response['status']['message']['email'])) {
        					if(is_array($response['status']['message']['email'])) {
        						gs_yotpo_display_message($response['status']['message']['email'][0], false);
        					}
        					else {
        						gs_yotpo_display_message($response['status']['message']['email'], false);
        					}        					
        				}   
        				else {
        					gs_yotpo_display_message($response['status']['message'], true);	
        				}    				
        	...
        	gs_yotpo_display_message($e->getMessage(), true);	
        }         		
	}
	else {
		gs_yotpo_display_message($errors, false);	
	}	
	return false;		
}

function gs_yotpo_display_message($messages = array(), $is_error = false) {
